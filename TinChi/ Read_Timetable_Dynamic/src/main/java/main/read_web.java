...
//			
//			// chọn vào tuần học
//            page.locator("#fullScreen > div.card-body.p-0 > div.row.text-nowrap.px-1.pb-1 > div.d-inline-block.col-lg-7.col-md-12.col-sm-12.mb-1 > ng-select > div > div > div.ng-input").click();
...
//            String weekText = page.locator("//div[@class='ng-dropdown-panel-items scroll-host']//div[contains(@class, 'ng-option')][1]").textContent().trim();
...
//			
//			// Chọn học ki
//			page.locator("body > app-root:nth-child(1) > div > div > div > div.contentshow.ng-star-inserted > div > div > div.px-md-0.frame_left > app-tkb-hocky > div > div.card-body.p-2 > div:nth-child(1) > div > ng-select > div").click();
//	        page.waitForTimeout(2000); 
//	        
//	        // Chọn ki 2
...
//            
//	        // Chọn table học kì và lấy dữ liệu 
//	        String tableSelector = "//table[contains(@class, 'table table-sm table-hover table-bordered table-sticky-header')]";
//            page.waitForSelector(tableSelector, new Page.WaitForSelectorOptions().setState(WaitForSelectorState.VISIBLE).setTimeout(60000));
//            String tableHtml = page.locator(tableSelector).innerHTML();
//	        System.out.println(tableHtml);
//	        browser.close();
//	        
//            
//	        if (tableHtml.contains("Không tìm thấy dữ liệu")) {
//                System.out.println("Học kỳ 2 - Năm học 2024 - 2025 không có dữ liệu thời khóa biểu. Vui lòng thử học kỳ khác.");
//                browser.close();
//                return;
//            }
//		}
...
public class read_web {
    ...
    public static WebData fetchData() throws InterruptedException {
        try (Playwright playwright = Playwright.create()) {
            Browser browser = playwright.chromium().launch(new BrowserType.LaunchOptions().setHeadless(false));
            Page page = browser.newPage();
