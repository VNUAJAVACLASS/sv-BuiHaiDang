...
public class read_web {
	...
	public static WebData fetchData() throws Exception {
		try (Playwright playwright = Playwright.create()) {
			...
			System.out.print("Nhập mã sinh viên (username): ");
			String msv = scanner.nextLine();

			// Nhập mật khẩu với hiển thị dấu sao
			String mk = readPasswordWithSwing();
	...
	private static String readPasswordWithSwing() {
	    JPasswordField pf = new JPasswordField();
	    int okCxl = JOptionPane.showConfirmDialog(null, pf, "Nhập mật khẩu", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
	    
	    if (okCxl == JOptionPane.OK_OPTION) {
	        char[] password = pf.getPassword();
	        return new String(password);
	    } else {
	        return ""; // Trả về chuỗi rỗng nếu người dùng hủy
	    }
	}
